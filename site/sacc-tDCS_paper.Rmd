...
author:
  ...
  - name: Tomas Knapen
    affiliation: VU,IBBA
  - name: Floris J.F.W. Roelofs
    affiliation: UvA
  - name: Richard Ridderinkhof
    affiliation: UvA,ABC
  - name: Heleen A. Slagter
    affiliation: UvA,ABC
address:
  - code: UvA
    address: Department of Psychology, University of Amsterdam, Amsterdam, The Netherlands
  - code: ABC
    address: Amsterdam Brain and Cognition (ABC), University of Amsterdam, Amsterdam, The Netherlands
  - code: VU
    address: Department of Experimental and Applied Psychology, Vrije Universiteit Amsterdam, Amsterdam, Netherlands
  - code: IBBA
    address: Institute for Brain and Behavior Amsterdam, Amsterdam, Netherlands
abstract: |
...
bibliography: sacc-tDCS.bib
csl: frontiers-in-neuroscience.csl
link-citations: true
colorlinks: true
numbersections: true
output:
...
Neuronavigation was always performed at the start of the first session (see [Frontal eye field localization]), and was usually not repeated on the second session. Otherwise, the procedure for each session was identical. First, a brief trial stimulation allowed participants to experience the sensations induced by tDCS and to decide whether they wanted to continue with the experiment (see [tDCS]). After setting up the eye tracker (see [Eye tracking]), participants practiced the prosaccade task (see [Task]) for one block (120 trials). Subsequently, participants performed 12 blocks of the task in three phases (Figure \@ref(fig:sacc-tDCS-procedure)): 3 blocks prior to stimulation (_baseline_), 3 blocks during stimulation (anodal or cathodal tDCS), and 6 blocks after stimulation (_post-1_ and _post-2_). Each block lasted approximately 5 minutes.

```{r sacc-tDCS-procedure, fig.cap='(ref:caption-figure-sacc-tDCS-procedure)', echo=FALSE}
knitr::include_graphics("figures/figure_1_procedure.png")
```
...
After task performance was complete, participants filled in a questionnaire on the occurrence of adverse effects related to tDCS (see Figure \@ref(fig:fig-sacc-tDCS-AE) in the [Supplementary material](#sacc-tDCS-supplement)).
...
```{r sacc-tDCS-task, fig.cap='(ref:caption-figure-sacc-tDCS-task)', echo=FALSE}
knitr::include_graphics("figures/figure_2_task.png")
```
...
Data were analyzed using the R programming language [@R2017] and several general packages [@Wickham2017; @Wickham2017a; @Robinson2017; @Wilke2016] from within RStudio [@RStudio2016].
...
Figure \@ref(fig:FEF) shows the MNI coordinates of the presumed right frontal eye field. While there is some spread (see Table \@ref(tab:MNI) in the [Supplementary Material](#sacc-tDCS-supplement) for the coordinates of all participants), the average coordinate (31.5, -1.8, 51.6) matched the anatomical definition we used for the individual MRIs: slightly anterior to the pre-central sulcus and slightly lateral to the superior frontal sulcus. The average coordinate also lies close to the one used in @Kanai2012, which was taken from @Paus1996 (31.3, -4.5, 50.9).

```{r FEF, fig.cap='(ref:caption-figure-FEF)', echo=FALSE}
knitr::include_graphics("figures/figure_3_FEF.png", dpi = 300)
```
...
```{r fig-latency, fig.cap='(ref:caption-figure-latency)', echo=FALSE}
knitr::include_graphics("figures/figure_4_latency.png")
```
...
```{r quantiles, fig.cap='(ref:caption-figure-quantiles)', echo=FALSE}
knitr::include_graphics("figures/figure_5_quantiles.png")
```
...
```{r fig-deviation, fig.cap='(ref:caption-figure-deviation)', echo=FALSE}
knitr::include_graphics("figures/figure_6_deviation.png")
```
...
```{r fig-variability, fig.cap='(ref:caption-figure-variability)', echo=FALSE}
knitr::include_graphics("figures/figure_7_variability.png")
```
...
LR and HS conceived the study. LR, TK, and HS designed the study. LR and FR collected the data. LR analyzed the data and wrote the first draft of the manuscript. LR, HS, KR, and TK interpreted the data and revised the manuscript. All authors read and approved the submitted version.

# Funding {-}

This work was supported by a VIDI and a Research Talent grant from the Netherlands Organization for Scientific Research (NWO) to HS and KR.
...
We thank Monja Hoven and Floortje Bouwkamp for their assistance in piloting and data collection. The following colleagues graciously shared their MRI data with us for neuronavigation purposes: Daan van Es, Anouk van Loon, Poppy Watson, Suzanne Oosterwijk, Yaïr Pinto, and Henk Cremers.

# Supplementary material {- #sacc-tDCS-supplement}

## tDCS adverse effects

```{r fig-sacc-tDCS-AE, fig.cap='(ref:caption-fig-sacc-tDCS-AE)', results='show', out.extra='', fig.pos='H'}
knitr::include_graphics("figures/figure_S1_sensations.png", auto_pdf = TRUE)
```
