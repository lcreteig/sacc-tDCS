...
## Bayesian linear mixed effects - saccade latency
...
The winning model is the one with both main effects, with a Bayes factor of `r extractBF(bfKanaiNull[1], onlybf = TRUE)`. We can compute the evidence for a particular effect by comparing this winning model with the best-fitting model that does _not_ contain the effect. We can compute the evidence for _absence_ of a particular effect by comparing the winning model with the best-fitting model that _does_ contain the effect.

* STIMULATION. This effect can be quantified by comparing the Bayes factors of the first and the 10th model : `r extractBF(bfKanaiNull[1] / bfKanaiNull[10], onlybf = TRUE)`. This constitues __strong evidence for the presence of a stimulation effect__.
* DIRECTION. This effect can be quantified by comparing the first and the 2nd model : `r extractBF(bfKanaiNull[1] / bfKanaiNull[2], onlybf = TRUE)`. This constitues __anecdotal evidence for the presence of a direction effect__.

* Interaction of STIMULATION and DIRECTION. This term is not in the winning model, and first shows up in the fourth model. The Bayes factor for the comparison of both models is `r extractBF(bfKanaiNull[1] / bfKanaiNull[4], onlybf = TRUE)`. This constitues __moderate evidence for the absence of an interaction between stimulation and direction__.
* Interaction of STIMULATION, and LEG. Bayes factor: `r extractBF(bfKanaiNull[1] / bfKanaiNull[8], onlybf = TRUE)`. This constitutes __strong evidence for the absence of an interaction between stimulation and leg__.
* Three-way interaction. Bayes factor: `r extractBF(bfKanaiNull[1] / bfKanaiNull[11], onlybf = TRUE)`. This constitutes __strong evidence for the absence of an interaction between stimulation, leg and direction__.
...
Removing the DIRECTION effect from the model yields a lower Bayes Factor, so including this effect improved the model. This thus constitues (a little bit of) evidence for the alternative: ` 1 \ ` `r extractBF(bfKanaiFull[6], onlybf = TRUE)` ` = ` `r extractBF(1/bfKanaiFull[6], onlybf = TRUE)`. The evidence for a stimulation effect is again pretty large: `r extractBF(1/bfKanaiFull[7], onlybf = TRUE)`.

On the contrary, removing the interactions actually improves the model, so there is evidence for the absence of interaction effects.

All in all, for most effects, the result is qualitatively similar to the approach of testing against the null-model. Only the evidence for absence of the interactions is much smaller here.
