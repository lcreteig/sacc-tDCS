% Read in MNI coords as subject by (X,Y,Z) matrix
coords = dlmread('FEF_coords_MNI_excl.csv',';',1,3);
coords(:,4) = 1:length(coords); % 4th column is color: assign different value to each subject
coords(:,5) = ones(1,length(coords)); % 5th column is node size: 1 mm for each subject

% Create spheres based on coordinates,
% plot in Surf Ice (https://www.nitrc.org/projects/surfice/)
nii_coord2spheres(coords,'FEF.node')

% function from https://github.com/rordenlab/spmScripts/blob/master/nii_coord2spheres.m
% discovered on https://www.nitrc.org/forum/message.php?msg_id=21772
...
if ~exist('exename','var') 
   exename = '/Applications/Surfice/surfice.app/Contents/MacOS/surfice';
end
