...
groupData <- read_csv(dataFile, na = "NaN")
groupData <- select(groupData, -X12) # drop final empty column named "X12"
```

```{r Make separate leg out of final 3 post blokcs}
# Load the data frame
groupData <- mutate(groupData, leg=replace(leg, leg == "post", "post.1"), # split the "post" level into two
...
```{r Quantiles and shift function}
qData <- groupData %>%
...
This takes quite a while to compute with a large number of bootstrap samples, so write the data file to disk:

```{r Write quantile data}
...
```{r Shift tDCS, lateral, left}
shiftTdcsLateralLeft <- ggplot(filter(qData, leg == "tDCS", type  == "lateral", direction == "left"), aes(anodal, difference)) +
...
```{r Attempt at fancy shift function}

fancyShift <- ggplot(filter(qStatsFancy,leg=="pre"), aes(anodal, difference)) +
  facet_grid(type ~ direction) +
  ...
  stat_summary(fun.data = mean_cl_normal, geom = "linerange", aes(colour = factor(sign), alpha  = factor(deco))) +
  scale_color_manual(values = c("#DD1C1A", "#06AED5"), guide = FALSE) +
  scale_alpha_discrete(range = c(0.4, 1), guide = FALSE) +
  # Line
  stat_summary(fun.y = mean, geom = "line", colour = "grey50", alpha = 0.5) +
  # Points
  stat_summary(fun.y = mean, geom = "point", colour = "black", fill = "white", shape = 21) +
  stat_summary(fun.y = mean, geom = "point", aes(fill = factor(sign), alpha = factor(deco)), colour = "black", shape = 21) +
  scale_fill_manual(values = c("#DD1C1A", "#06AED5"), guide = FALSE) +
  xlab("anodal deciles") +
  ylab("anodal - cathodal deciles")
fancyShift
```